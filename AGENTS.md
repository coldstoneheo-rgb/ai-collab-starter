...
workflow 단계에서 실패를 성공으로 위장하는 fallback (`|| echo`, `continue-on-error: true`) 사용

main/master 대상 자동 commit/push 자동화
...
$GITHUB_OUTPUT는 `echo "key=value" >> $GITHUB_OUTPUT` 방식만 허용

중간 파일 사용 ❌

router 결과를 workflow에서 재판단하는 추가 조건 로직 ❌
...
**중요 해석 규칙:**
- workflow의 `if:`는 router가 이미 결정한 `enabled_agents`를 실행 게이트로만 사용한다.
- workflow에서 새로운 판단 근거(파일 경로, 비용, 위험도)를 추가하면 규칙 위반이다.
...
- 월 예산 초과 상태에서는 enterprise 모드 실행 금지
- 동일 PR에서 재시도는 최대 1회, 이후 사람 검토 전환
- 고비용 모델 호출 전에는 저비용 모드(lite/pro)로 선검토
- 비용 리포트(`.ai/budget.json`) 갱신 실패 시 AI 실행 중단

### 16.3 유지보수 규칙

- `mode_map`, `router`, `workflow`는 항상 동기화되어야 함
- 프롬프트 파일명은 `_vN` 규칙 강제, 구버전 삭제 금지
- 러너가 없는 agent는 `enabled_agents`에 포함 금지

### 16.4 안정성 규칙

- 실패를 성공으로 덮는 fallback 금지
- 민감 경로 변경 시 autofix 생성 자체 금지
- `DISABLE_AI_AUTOMATION=true`면 모든 runner 즉시 skip
